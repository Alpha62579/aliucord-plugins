...
public class ModPage extends SettingsPage {
    ...
    public static void banUser(User user, long guildID) {
        new Thread(() -> RxUtils.subscribe(RestAPI.api.banGuildMember(guildID, user.getId(), new RestAPIParams.BanGuildMember(0), null), RxUtils.createActionSubscriber(onNext -> {
        }, onError -> Utils.showToast("Failed to ban " + user.getUsername())))).start();
    }

    public static void kickUser(User user, long guildID) {
        new Thread(() -> RxUtils.subscribe(RestAPI.api.kickGuildMember(guildID, user.getId(), null), RxUtils.createActionSubscriber(onNext -> {
        }, onError -> Utils.showToast("Failed to kick " + user.getUsername())))).start();
    }
    ...
    public void onViewBound(View view) {
        ...
        kickHeader = addHeaderButton("Kick ALL", drawable, onclick -> {
            ...
            dialog.setOnOkListener(view1 -> memberList.forEach(user -> kickUser(user, guildID)));
        ...
        banHeader = addHeaderButton("Ban ALL", drawable, onclick -> {
            ...
            dialog.setIsDangerous(true);
            dialog.setOnOkListener(view1 -> memberList.forEach(user -> banUser(user, guildID)));
            dialog.setOnCancelListener(view1 -> dialog.dismiss());
        ...
        tempView.setTypeface(ResourcesCompat.getFont(ctx, Constants.Fonts.whitney_bold));
        addView(tempView);
        memberList.forEach(user -> addView(new MemberView(ctx, user, guildID, this, view)));
    }
}
